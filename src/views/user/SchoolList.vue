    ...
    <el-container>
      <el-header style="box-shadow: 0 2px 4px rgba(0, 0, 0, .12), 0 0 6px rgba(0, 0, 0, .04)">
        <el-row style="width:100%">
          <el-col :span="21">升学信息管理</el-col>
          <!-- <el-col :span="3">
            <el-button @click="dialogAddVisible = true" type="text" style="color:#303133">添加章节</el-button>
          </el-col>-->

          <el-col :span="3">
            <el-button @click="addRoute" type="text" style="color:#303133">新增升学信息</el-button>
          </el-col>
        </el-row>
      </el-header>
      <el-main
...
export default {
  ...
  methods: {
    ...
    handlerUpdateRoute(index, row) {
      this.$router.push("/editSchool/" + row.id);
    },
    handlerReadRoute(index, row) {
      this.$router.push("/school/" + row.id);
    },
    async getDocs() {
      let res = await this.$http.get("/get_docs/3");
      if (res != null) {
        console.log(res);
