      ...
      >
        <el-card shadow="hover" style="height:82%">
          <div slot="header" class="clearfix">
            <span style="padding: 3px 0; font-size:30px" type="text">评估历史</span>
          </div>
          <el-table
            :data="exams"
            height="250"
...
export default {
  ...
  methods: {
    async add_exam() {
      ...
      let res = await this.$http.post("/begin_exam", this.addForm);
      if (res != null) {
        console.log(res);
        this.$router.push("/exam/" + res.data.exam.id);
      }
    },
    handleEdit(index, row) {
      this.$router.push("/exam/" + row.id);
    }
  },
  async created() {
